...
import NotifyRulesEditor from '../NotifyRulesEditor.vue'
...
const form = ref({ number: '', name: '', header: '', custom_notify: '' })
const assignFor = ref<NumberRow | null>(null)
const assignSel = ref<number[]>([])

async function load() {
  try {
    numbers.value = await api<NumberRow[]>(`/admin/numbers?org_id=${props.orgId}`)
...
async function create() {
  ...
  busy.value = true
  try {
    await api('/admin/numbers', { json: { ...form.value, org_id: props.orgId } })
    form.value = { number: '', name: '', header: '', custom_notify: '' }
    await load()
...
function numberActions(n: NumberRow): ActionItem[] {
  return [
    { label: 'Edit', onClick: () => edit(n) },
    { label: 'Delete', danger: true, onClick: () => remove(n) },
  ]
}
</script>
...
<template>
  ...
  <div class="panel">
    <table>
      <thead>
        <tr><th>Number</th><th>Name / Header</th><th>Upstream</th><th>Status</th><th>Inbox</th><th>Assigned</th><th></th></tr>
      </thead>
      <tbody>
        <tr v-for="n in numbers" :key="n.id">
          ...
          <td class="actions-cell">
            ...
            <ActionsMenu :items="numberActions(n)" />
          </td>
        </tr>
        <tr v-if="!numbers.length"><td colspan="7" class="muted">No numbers for this organization yet.</td></tr>
      </tbody>
    </table>
  </div>
