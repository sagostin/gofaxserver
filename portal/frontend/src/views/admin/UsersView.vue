...
const form = ref({ username: '', email: '', password: '', role: 'user', org_id: 0 })
...
async function create() {
  ...
  try {
    ...
    form.value = { username: '', email: '', password: '', role: 'user', org_id: 0 }
    await load()
...
<template>
  <main class="page">
    ...
    <div class="panel">
      <table>
        <thead>
          <tr><th>Username</th><th>Email</th><th>Role</th><th>Org</th><th>Numbers</th><th>Status</th><th></th></tr>
        </thead>
        <tbody>
      ...
      <p class="muted">Assign outbound numbers under the Numbers tab — that also wires the receipt emails.</p>
    </div>
  </main>
</template>
