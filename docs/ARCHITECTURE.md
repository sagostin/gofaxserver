...
| **Gateway Monitor** | `gofaxserver/gateway_monitor.go` | Periodically polls `sofia status gateway` for provisioned `register=true` gateways, records `last_state`, and logs registration transitions (drops at WARN, recoveries at INFO). Each tick also re-resolves hostname-valued gateway ACL entries (`Server.GatewayACLResolved`) so far-end IP changes are picked up automatically; last-good IPs are kept on DNS failure. Interval: `gateway_monitor_seconds` (default 60, negative disables). |
