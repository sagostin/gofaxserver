...
| **Gateway Provisioner** | `gofaxserver/gateway_provision.go`, `web_gateways.go` | API-driven FreeSWITCH gateway management: DB-backed templates rendered to `freeswitch.gateway_config_dir`, activated via ESL (`reloadxml` + `sofia profile fax rescan`), combined with endpoint creation. Secrets encrypted at rest with `psk`. |
