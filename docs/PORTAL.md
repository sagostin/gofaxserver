...
`GET /portal/api/admin/fax-results[?tenant_id=&result_type=&success=&origin=&number=&job_uuid=&from=&to=&limit=&offset=]`
(grouped all-jobs view; `origin=portal|non-portal` filters by intake source,
portal jobs carry `portal` + `portal_org`/`portal_user` annotations),
